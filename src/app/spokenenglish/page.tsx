      ...
      </div>
      {
        spokeneng.length > 0 && spokeneng.map((item) => (
          <div key={item.id} className='grid grid-cols-9 p-4 m-2 font-medium text-sm overflow-hidden break-words bg-white shadow-md'>
            ...
            <p>{item.price}</p>
            <p>{item.url}</p>
            <button className='flex items-center justify-center'><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.7} stroke="currentColor" className="size-6 rounded-full text-red-600">
